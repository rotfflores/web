(() => {
  ...
  $$(".bouquet").forEach((image) => {
    image.alt = image.classList.contains("bouquet--final")
      ? `Ramo final de girasoles para ${config.recipient}`
      : `Ramo de girasoles amarillos para ${config.recipient}`;
  });
  ...
  const daysSongToggle = $("#days-song-toggle");
  ...
  function updateMusicUI() {
    ...
    if (daysSongToggle) {
      daysSongToggle.classList.toggle("is-playing", playing);
      daysSongToggle.setAttribute("aria-label", playing ? "Pausar Nuestra canción" : "Reproducir Nuestra canción");
    }
  }
  ...
  async function toggleMusic(forcePlay = false) {
    ...
    try {
      ...
      if (error?.name !== "NotAllowedError" && error?.name !== "AbortError") {
        ...
        $(".music-control__label", musicControl).textContent = "Canción no disponible";
      }
    }
    updateMusicUI();
  }
  ...
  daysSongToggle?.addEventListener("click", () => toggleMusic());
  ...
  $("#open-experience").addEventListener("click", () => {
    ...
    setTimeout(() => {
      ...
      observeReveals();
      $("#story-announcer").textContent = `Historia 1 de ${$$("#experience > section").length}. Para ${config.recipient}.`;
      const firstTitle = $("#hero-title");
      firstTitle.tabIndex = -1;
  ...
  const maxMemoryPhotos = 8;
  const photos = (config.photos || []).slice(0, maxMemoryPhotos);
  const defaultCaptions = [
    "Donde todo comenzó.",
    "Uno de mis días favoritos.",
    "Tu sonrisa favorita.",
    "Y todavía nos faltan muchos recuerdos.",
    "Cada instante contigo se vuelve especial.",
    "Un recuerdo que quiero guardar siempre.",
    "La mejor parte de mi día eres tú.",
    "Porque contigo, cada recuerdo vale la pena."
  ];
  const memoryPhotos = photos.length ? Array.from({ length: maxMemoryPhotos }, (_, index) => ({
  ...
  let photoLastFocused = null;
  let photoCloseTimer = 0;

  function setModalBackgroundInert(isInert) {
    [$("#experience"), $("#story-progress"), $("#story-next")].forEach((element) => {
  ...
  if (scratchCanvas && "ResizeObserver" in window) {
    let scratchResizeFrame = 0;
    new ResizeObserver(() => {
      if (scratching || scratchComplete || scratchedCells.size) return;
      cancelAnimationFrame(scratchResizeFrame);
      scratchResizeFrame = requestAnimationFrame(drawScratchCover);
  ...
  document.addEventListener("keydown", (event) => {
    const openDialog = [letterModal, photoModal].find((dialog) => dialog.classList.contains("is-open"));
    if (event.key === "Escape") {
    ...
    if (event.key === "Tab" && openDialog) {
      const focusable = $$("button:not([disabled]):not([hidden]), a[href], [tabindex]:not([tabindex='-1'])", openDialog)
  ...
  const daysStat = $("#days-together-stat");
  let daysAnimation = 0;
  ...
  function setupWrappedStories() {
    ...
    const cardTitle = (card) => {
      const heading = $("h1, h2", card);
      if (!heading) return "";
      const readableHeading = heading.cloneNode(true);
      $$("br", readableHeading).forEach((breakElement) => breakElement.replaceWith(" "));
      return (readableHeading.textContent || "").replace(/\s+/g, " ").trim();
    };
    ...
    cards.forEach((card, index) => {
      ...
      segment.setAttribute("aria-label", `Ir a la historia ${index + 1}${heading ? `: ${heading}` : ""}`);
    ...
    function showCard(index) {
      ...
      const previousIndex = activeCard;
      const movingForward = previousIndex === cards.length - 1 && nextIndex === 0
        ? true
        : previousIndex === 0 && nextIndex === cards.length - 1
          ? false
          : nextIndex > previousIndex;
      const shouldAnimateCards = nextIndex !== previousIndex
        && experience.getAttribute("aria-hidden") === "false"
        && !reducedMotion;

      clearTimeout(cardTransitionTimer);
      experience.classList.remove("is-card-transitioning", "is-going-forward", "is-going-back");
      cards.forEach((card) => card.classList.remove("is-card-entering", "is-card-leaving"));
      ...
      document.body.dataset.story = String(activeCard + 1).padStart(2, "0");
      ...
      if (shouldAnimateCards) {
        experience.classList.add("is-card-transitioning", movingForward ? "is-going-forward" : "is-going-back");
        cards[previousIndex].classList.add("is-card-leaving");
        cards[activeCard].classList.add("is-card-entering");
      ...
      const cardLocked = (activeCard === 3 && !reasonsUnlocked) || (activeCard === 5 && !giftUnlocked);
      ...
      else window.stopYellowUniverseIntro?.();
      ...
      if (experience.getAttribute("aria-hidden") === "false") {
        const title = cardTitle(cards[activeCard]);
        $("#story-announcer").textContent = `Historia ${activeCard + 1} de ${cards.length}${title ? `. ${title}` : ""}.`;
      }
    }
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if ($(".modal.is-open, .photo-modal.is-open")) return;
      if (event.defaultPrevented || event.altKey || event.ctrlKey || event.metaKey) return;
      if (event.target.closest?.("button, a, input, textarea, select, [contenteditable='true'], canvas, iframe")) return;
    ...
    experience.addEventListener("pointercancel", () => { touchStart = null; });
