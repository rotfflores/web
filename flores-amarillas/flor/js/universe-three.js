(() => {
  ...
  window.createYellowUniverse3D = ({ config, reducedMotion }) => {
    ...
    const pixelRatioLimit = innerWidth < 640 || (navigator.deviceMemory && navigator.deviceMemory <= 4) ? 1.35 : 1.75;
    ...
    function render(time = 0) {
      const delta = previousRenderTime ? Math.min(32, time - previousRenderTime) : 16;
      ...
      words.forEach((sprite) => {
        if (!reducedMotion) sprite.userData.theta += sprite.userData.speed * delta;
      ...
      rings.children[0].rotation.z += reducedMotion ? 0 : .00042 * frameScale;
      rings.children[1].rotation.z -= reducedMotion ? 0 : .0003 * frameScale;
      rings.children[2].rotation.z += reducedMotion ? 0 : .0002 * frameScale;
    ...
    const stopIntro = (revealUniverse = false) => {
      ...
      section.classList.toggle("is-universe-waiting", !revealUniverse);
      section.classList.toggle("is-universe-visible", revealUniverse);
    };
    window.stopYellowUniverseIntro = () => stopIntro(false);
